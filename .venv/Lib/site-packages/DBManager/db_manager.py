...
class ProductRecords:
    ...
    def add(self, product_name, barcode, purchase_price, selling_price, piece, unit, user, process_type, customer, payment_status, description=None, time=None, date=None):
        now = datetime.now()
        if time is None:
        ...
        self.cursor.execute('''INSERT INTO products (
            product_name, barcode, purchase_price, selling_price, piece, unit, time, date, user, process_type, customer, payment_status, description
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
        (product_name, barcode, float(purchase_price), float(selling_price), int(piece), unit, time, date, user, process_type, customer, payment_status, description))
        self.conn.commit()
    ...
    def find_row_id(self, product_name, barcode, purchase_price, selling_price, piece, unit, time, date, user, process_type, customer, payment_status, description):
        self.cursor.execute('''SELECT ROWID FROM products WHERE
            product_name=? AND barcode=? AND purchase_price=? AND selling_price=? AND piece=? AND unit=? AND time=? AND date=? AND user=? AND process_type=? AND customer=? AND payment_status=? AND description=?''',
            (product_name, barcode, purchase_price, selling_price, piece, unit, time, date, user, process_type, customer, payment_status, description))
