...
class ProductRecords:
    ...
    def refresh_for_time(self):
        ...
        self.cursor.execute('''
            ...
            ORDER BY date(date, 'dd/mm/yyyy') ASC, time ASC
        ''')
    ...
    def find_row_id(self, product_name, barcode, purchase_price, selling_price, piece, unit, time, date, user, process_type, customer, payment_status):
        self.cursor.execute('''SELECT ROWID FROM products WHERE
            product_name=? AND barcode=? AND purchase_price=? AND selling_price=? AND piece=? AND unit=? AND time=? AND date=? AND user=? AND process_type=? AND customer=? AND payment_status=?''',
            (product_name, barcode, purchase_price, selling_price, piece, unit, time, date, user, process_type, customer, payment_status))
    ...
    def get_by_date(self, date_dict):
        ...
        return self.cursor.fetchall()
    def get_customers_by_process_type(self, process_type):
        self.cursor.execute('''
            SELECT DISTINCT customer FROM products WHERE process_type=?
        ''', (process_type,))
        return [row[0] for row in self.cursor.fetchall()]
