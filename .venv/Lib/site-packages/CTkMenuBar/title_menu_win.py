...
from .dropdown_menu import CustomDropdownMenu as DropdownMenu
...
class CTkTitleMenu(customtkinter.CTkToplevel):
    ...
    def __init__(
        ...
        if master_type=="tk":
            ...
            pass 
        elif master_type.startswith("!new_toplevel") :
            pass   
        else:
            print("Only root windows/toplevels can be passed as the master!")
    ...
    def add_cascade(self, text=None, postcommand=None,dropdown_option: dict = {None:None} ,**kwargs):
        if "hover_color" in kwargs:
            if "transparentcolor" == kwargs["hover_color"]:
                kwargs["hover_color"]=self.transparent_color
        ...
        #self.dropdown_menus[text]=DropdownMenu(self.menu_buttons[text],**dropdown_option)
        if dropdown_option!={None:None}:
            self.dropdown_menus : Dict[str , DropdownMenu] ={text:DropdownMenu(self.menu_buttons[text],**dropdown_option)}
            self.last_dropdown = self.dropdown_menus[text]
        return self.menu_button
    def add_entry(self,width=25,**kwargs):
        if "hover_color" in kwargs:
            if "transparentcolor" == kwargs["hover_color"]:
                kwargs["hover_color"]=self.transparent_color
        if not "fg_color" in kwargs:
        ...
        self.menu_button = customtkinter.CTkEntry(self, fg_color="#000000",
                                                   text_color=text_color,justify="center", corner_radius=5,border_width=1,border_color="#aaaaaa",width=width, height=10, **kwargs)
        self.menu_button.grid(row=0, column=self.num, padx=(0, self.padding))
        self.menu_button.bind('<FocusIn>', lambda event: self.after(5000, lambda:self.focus()))
        self.menu_button.insert(0,"1")
        self.num += 1
